...
for n in sizes:
    ...
    #for i in important_idx:
        #print(i, diff_vec[i])
    important_idx = [0, 1, 2, 73, 140, 205, 256, 257]
    vals = {i: diff_vec[i] for i in important_idx}
    dominant = max(vals, key=vals.get)
    #print("dominant_idx:", dominant, "| value:", vals[dominant])

    #print(f"{n:6d} | B: {b_after:.4f} | Δimp:",
          #diff_vec[important_idx])
          
    delta = abs(b_after - b_before)
    print(n, "| B:", b_after)
    if delta > THRESH:
        print("FIRST SIGNIFICANT CHANGE AT:", n)
        print("dominant_idx:", dominant)
        break
